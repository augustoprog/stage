...
if (document && folder) {
  ...
  var androidCheckboxes = createSelectionPanel(
    "Android",
...
function exportToFile(scaleFactor, resIdentifier, os) {
  ...
  if (os === "android")
    expFolder = new Folder(
      folder.fsName + "/Android/drawable-" + resIdentifier
    );
  else if (os === "ios") expFolder = new Folder(folder.fsName + "/iOS");
  ...
  for (i = document.artboards.length - 1; i >= 0; i--) {
    ...
    if (ab.name.charAt(0) == "!") continue;

    if (os === "android")
      file = new File(expFolder.fsName + "/" + ab.name + ".png");
    else if (os === "ios")
      file = new File(
        expFolder.fsName + "/" + ab.name + resIdentifier + ".png"
      );
